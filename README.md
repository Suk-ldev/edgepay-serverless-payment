...
后台“使用文档”已经逐项列出商业插件和免费收钱吧的凭据来源、字段填写、Webhook、Worker/Docker 运行位置和排错步骤。

管理后台每次打开都会检查本仓库的最新商业发行版本。发现新版本后可直接进入部署向导，填写原 Worker 名称并选择“无损升级”；升级只替换程序文件，原 D1、插件配置、支付通道、环境变量、Secrets、定时任务和访问路由不变。
